...
function initializeUser() {
    ...
    user.transaction(
                function(tx) {
                    tx.executeSql('CREATE TABLE IF NOT EXISTS articles(id TEXT UNIQUE, title TEXT, url TEXT, image TEXT, domain TEXT, time TEXT, is_fav TEXT, is_archive TEXT, sort_id TEXT, is_video TEXT, html TEXT, host TEXT, datePublished TEXT)');
                });
}
...
function list_got(results) {
    ...
    for (var key in results['list']) {
        sort_array.push({key:key, sort_id:results['list'][key]['sort_id']});
    }

    sort_array.sort(function(x,y){return x.sort_id - y.sort_id});

    for (var i=0;i<sort_array.length;i++) {
        var item = results['list'][sort_array[i].key];

        var image = '';
        if (item['has_image'] == '1' && item['image']) {
            image = item['image']['src'];
        } else {
            image = '';
        }
        //results['list'][i]['resolved_title'] = results['list'][i]['resolved_title'].replace('\n', ' ');
        var only_domain = extractDomain(item['resolved_url']);

        var is_video = item['has_video'] && item['has_video'] != '0' ? '1' : '0';

        pockethomeitemsModel.append({"item_id":item['item_id'], "title":item['resolved_title'], "url":item['resolved_url'], "image":image, "domain":only_domain, "time_added":item['time_added'], "is_fav":item['favorite'], "sort_id":item['sort_id'], "is_video":is_video});
    }
...
function archive_got(results) {
    ...
    for (var i=0;i<sort_array.length;i++) {
        ...
        archivehomeitemsModel.append({"item_id":item['item_id'], "title":item['resolved_title'], "url":item['resolved_url'], "image":image, "domain":only_domain, "time_added":item['time_added'], "is_fav":item['favorite'], "sort_id":item['sort_id'], "is_video":is_video});
    }
...
function favorites_got(results) {
    ...
    for (var i=0;i<sort_array.length;i++) {
        ...
        //item['resolved_title'] = item['resolved_title'].replace('\n', ' ');
        var only_domain = extractDomain(item['resolved_url']);
