...
Item {
    ...
    function get_list() {
        Scripts.get_favorites();
    }

    function get_search(query) {
        Scripts.get_search(query, 'favorites');
    }

    ListModel {
        id: favoriteshomeitemsModel
    }

    ListView {
        id:favoriteshomeitems
        anchors.top: parent.top
        ...
        delegate: ListItemWithActions {
            ...
            onSwippingChanged: {
                favoriteshomeitems._updateSwipeState(itemsDelegate)
            }
        ...
        PullToRefresh {
            refreshing: favoriteshomeitemsModel.count == 0 && favoriteshomeitems.status == 0
            onRefresh: get_list()
        }
    }
    ...
    Item {
        id: indicat
        anchors.centerIn: parent
        opacity: favoriteshomeitemsModel.count == 0 && favoriteshomeitems.status == 0 ? 1 : 0
    ...
    Item {
        id: noitem
        visible: favoriteshomeitemsModel.count == 0 && favoriteshomeitems.status == 1 ? true : false
        ...
        Icon {
            ...
            width: units.gu(3)
            height: width
            name: favHome.state == "default" ? "starred" : "search"
        }
        ...
        Label {
            ...
            anchors.centerIn: parent
            fontSize: "large"
            text: favHome.state == "default" ?
                      i18n.tr("<b>YOU HAVE NO FAVORITES</b>") :
